...
// whole forward pass. These float32 versions are ~4-6× faster and accurate
// to ~3e-7 — three orders of magnitude inside the 1e-4 golden tolerance.
// Both are pinned against the float64 stdlib in fastmath_test.go.
...
// expf32 computes e^x for float32 with ~2e-7 relative error (Cephes expf:
// range reduction by ln2 in two parts, degree-5 polynomial, exponent
// reassembled through the float32 bit layout).
...
// measured ~3× faster than the called version per element. Inputs below
// the underflow clamp return exp(-87.33) ≈ 1.2e-38 rather than 0 — an
// absolute error < 1.3e-38, three dozen orders below the golden tolerance.
